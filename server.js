...
} else { console.log('Email ОТКЛЮЧЕН. Коды будут в консоли.'); }

const db = new Database('mixora.db');
db.pragma('foreign_keys = ON');

// ===== ВРЕМЕННО: ПЕРЕСОЗДАНИЕ ТАБЛИЦЫ USERS (УДАЛИТЬ ПОСЛЕ ПЕРВОЙ УСПЕШНОЙ РЕГИСТРАЦИИ) =====
    ...
    socket.on('register', data => {
        ...
        try {
            ...
            if (transporter) {
                ...
                socket.emit('verificationRequired', { email: data.email, message: 'Код в консоли сервера.' });
            }
        } catch (e) {
